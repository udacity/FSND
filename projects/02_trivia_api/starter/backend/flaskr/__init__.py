      ...
      if category==0:
        if idList is None:
          question=Question.query.first()
           ...
           question=Question.query.filter(Question.id.notin_(idList)).first() 
      else:
        if idList is None:
          question=Question.query.filter(Question.category==category).first()  
        else:
          question=Question.query.filter(Question.category==category).filter(Question.id.notin_(idList)).first()    
