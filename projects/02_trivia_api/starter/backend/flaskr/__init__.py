...
def create_app(test_config=None):
    ...
    def search_questions():
        ...
        term = "%"+term.strip()+"%"
        condition = Question.question.ilike(term)
        all_questions = Question.query.filter(condition).all()
        current_questions = paginate_questions(request, all_questions)
    ...
    def play_quiz():
        ...
        if category == 0:
            condition = Question.id.notin_(previous_questions)
            questions = Question.query.filter(condition).all()
        else:
            condition1 = Question.category == category
            condition2 = Question.id.notin_(previous_questions)
            questions = Question.query.filter(condition1, condition2).all()
