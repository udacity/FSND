...
def create_app(test_config=None):
  ...
  '''
    ...
    def handle_questions():
        if request.method == 'GET':
            args = {
                key: int(val) if key == 'page' else val for key,
                val in request.args.items()}
    ...
    def create_question(question, answer, difficulty, **kwargs):
        ...
        try:  # If no duplicate is found, create question
            dupes = Question.query.filter(
                Question.question.ilike(f"%{qst['question']}%")).all()
            if len(dupes) > 0:
                abort(422)
  ...
  '''
    ...
    def search_question():
        ...
        try:
            search_result = Question.query.filter(
                filter_on.ilike(f'%{data["searchTerm"]}%')).all()
    ...
    def search_categories():
        ...
        try:
            search_result = Category.query.filter(
                Category.type.ilike(f'%{data["searchTerm"]}%')).all()
  ...
  '''
    ...
    def get_random_question():
        ...
        available_questions = Question.query.filter(
            and_(for_categories, not_in_previous_questions)).all()
