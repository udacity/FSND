...
def search_venues():
  #query db for any venues matching search term
  #grab the search term
  search_term=request.form.get('search_term')

  #find all the results based on the search term (including partial and case sensitive)
  ...
  #store all matching venues to search term in response
  response = {
    "count": len(venues),
    "data": data
  }
...
def delete_venue(venue_id):
  try:
    ...
    #if error, flash error to alert the user
    flash('An error occured. Venue ' + venue_name + ' could not be deleted.')
  finally:
    db.session.close()
  return render_template('pages/home.html')
...
def search_artists():
  #query db for any artists matching search term
  #grab the search term
  search_term=request.form.get('search_term')
  ...
  #store all matching artists to search term in response
  response = {
    "count": len(artists),
    "data": data
  }
...
def edit_venue_submission(venue_id):
  #called upon submitting the edit venue listing form
  try:
    venue_form = VenueForm()
    venue = Venue.query.filter_by(id=venue_id).first()
    ...
    venue.seeking_desc = venue_form.seeking_desc.data
    ...
    db.session.rollback()
    flash('An error occured updating Venue ' + request.form['name'] + ". " + str(error))
  except:
    db.session.rollback()
