...
class Artist(db.Model):
    ...
    def past_events(self):
      now = datetime.now()
      # '%Y-%m-%dT%H:%M:%S.%fZ'
      past_events = [show for show in self.shows if datetime.strptime(show, '%Y-%m-%dT%H:%M:%S') < now]
      return past_events

    @property
    def past_event_count(self):
      return len(self.past_events)

    @property
    def future_events(self):
      now = datetime.now()
      future_events = [show for show in self.shows if datetime.strptime(show, '%Y-%m-%dT%H:%M:%S.%fZ') > now]
      return future_events

    @property
    def future_event_count(self):
