...
def create_app():
    ...
    def get_drinks_short():
        try:
            drinks_detailed = Drink.query   \
                .with_entities(Drink.id, Drink.title, Drink.recipe) \
    ...
    def get_drinks_long(jwt):
        try:
            ...
            for i in range(len(drinks_detailed)):
                ...
                drink = {
                    'id': drinks_detailed[i].id,
                    'title': drinks_detailed[i].title,
                    'recipe': recipe
                    }
                drinks.append(drink)
    ...
    def add_drink(jwt):
        ...
        try:
            new_drink.insert()
            drink = Drink.query.with_entities(Drink.id) \
                .filter(Drink.title == data['title']).one_or_none()
            return jsonify({
                'success': True,
                'drinks': [{
                    'id': drink.id,
    ...
    def delete_drink(jwt, id):
        try:
            drink = Drink.query    \
                .with_entities(Drink.id, Drink.title, Drink.recipe)  \
                .filter(Drink.id == id).one_or_none()
            if drink is None:
                abort(404)
